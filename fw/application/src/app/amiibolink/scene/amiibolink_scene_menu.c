...
const char *mode_name[] = {"", "Aleatorio(Manual)", "Por orden", "Leer y escribir", "Aleatorio(Automático)"};
...
void amiibolink_scene_menu_on_event(mui_list_view_event_t event, mui_list_view_t *p_list_view,
    ...
    if (event == MUI_LIST_VIEW_EVENT_SELECTED) {
        if (menu_code == AMIIBOLINK_MENU_BACK_MAIN) {
            ...
            sprintf(txt, "Aleatorio automático [%s]", p_settings->auto_gen_amiibolink ? "Abrir" : "Guan");
            settings_save();
...
void amiibolink_scene_menu_on_enter(void *user_data) {
    ...
    char txt[32];
    sprintf(txt, "Modo [%s]", mode_name[app->amiibolink_mode]);
    ...
    mui_list_view_add_item(app->p_list_view, ICON_VER, "Detalles de la etiqueta", (void *)AMIIBOLINK_MENU_BACK_MAIN);

    mui_list_view_add_item(app->p_list_view, ICON_HOME, ">>Menú principal<<", (void *)AMIIBOLINK_MENU_BACK_EXIT);
