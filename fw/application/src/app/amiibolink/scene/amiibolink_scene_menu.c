...
void amiibolink_scene_menu_on_enter(void *user_data) {
    ...
    char txt[32];
    sprintf(txt, "Mode [%s]", mode_name[app->amiibolink_mode]);
    ...
    mui_list_view_add_item(app->p_list_view, ICON_HOME, ">>Main menu<<", (void *)AMIIBOLINK_MENU_BACK_EXIT);
