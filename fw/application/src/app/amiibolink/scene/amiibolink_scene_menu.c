...
const char *mode_name[] = {"", "casuale(Manuale)", "In ordine", "Lettura e scrittura", "casuale(Auto)"};
...
void amiibolink_scene_menu_on_event(mui_list_view_event_t event, mui_list_view_t *p_list_view,
    ...
    if (event == MUI_LIST_VIEW_EVENT_SELECTED) {
        if (menu_code == AMIIBOLINK_MENU_BACK_MAIN) {
            ...
            sprintf(txt, "Random automatica [%s]", p_settings->auto_gen_amiibolink ? "aperto" : "chiudi");
            settings_save();
...
void amiibolink_scene_menu_on_enter(void *user_data) {
    ...
    char txt[32];
    sprintf(txt, "modalità [%s]", mode_name[app->amiibolink_mode]);
    ...
    sprintf(txt, "Modalità compatibilità [%s]",
    ...
    mui_list_view_add_item(app->p_list_view, ICON_VER, "Dettagli dell'etichetta", (void *)AMIIBOLINK_MENU_BACK_MAIN);

    mui_list_view_add_item(app->p_list_view, ICON_HOME, ">>Menu principale<<", (void *)AMIIBOLINK_MENU_BACK_EXIT);
