...
static void settings_scene_main_list_view_on_selected(mui_list_view_event_t event, mui_list_view_t *p_list_view,
    ...
    case SETTINGS_MAIN_MENU_SKIP_DRIVER_SELECT:
        ...
        sprintf(txt, "Selección automática [%s]", p_settings->skip_driver_select ? "Abrir" : "Guan");
    ...
    case SETTINGS_MAIN_MENU_LI_MODE:
        ...
        sprintf(txt, "Batería de litio [%s]", p_settings->bat_mode ? "Abrir" : "Guan");
        string_set_str(p_item->text, txt);
        mui_update(mui());
        break;
    ...
    case SETTINGS_MAIN_MENU_SHOW_MEM_USAGE:
        ...
        sprintf(txt, "Uso de la memoria [%s]", p_settings->show_mem_usage ? "Abrir" : "Guan");
        string_set_str(p_item->text, txt);
        mui_update(mui());
        break;
    ...
    case SETTINGS_MAIN_MENU_ENABLE_HIBERNATE:
        ...
        sprintf(txt, "Despertar rápido [%s]", p_settings->hibernate_enabled ? "Abrir" : "Guan");
        string_set_str(p_item->text, txt);
        mui_update(mui());
        break;
...
void settings_scene_main_on_enter(void *user_data) {
    ...
    char txt[32];
    sprintf(txt, "Versión [%s]", version_get_version(version_get()));
    ...
    if (p_settings->lcd_backlight == 0) {
        sprintf(txt, "Brillo [Guan]");
    } else {
        sprintf(txt, "Brillo [%d%%]", p_settings->lcd_backlight);
    }
    ...
    sprintf(txt, "Tiempo de hibernación [%ds]", nrf_pwr_mgmt_get_timeout());
    mui_list_view_add_item(app->p_list_view, 0xe1c9, txt, (void *)SETTINGS_MAIN_MENU_SLEEP_TIMEOUT);
    mui_list_view_add_item(app->p_list_view, 0xe1ca, "Actualización", (void *)SETTINGS_MAIN_MENU_DFU);
    mui_list_view_add_item(app->p_list_view, 0xe069, "Menú principal", (void *)SETTINGS_MAIN_MENU_EXIT);
