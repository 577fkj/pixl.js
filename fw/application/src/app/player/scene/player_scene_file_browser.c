...
static void player_scene_file_browser_reload_folders(app_player_t *app) {
    ...
    mui_list_view_add_item(app->p_list_view, ICON_HOME, ">>Menu principale<<", (void *)-1);
    ...
    if (res == VFS_OK) {
        ...
        mui_list_view_add_item(app->p_list_view, ICON_ERROR, "Apertura cartella non riuscita", (void *)-1);
    }

    if (file_cnt == 0) {
        mui_list_view_add_item(app->p_list_view, ICON_ERROR, "<Nessun file di animazione>", (void *)-1);
    }
