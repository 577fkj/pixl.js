...
static void amiibo_scene_storage_list_menu_on_selected(mui_list_view_event_t event, mui_list_view_t *p_list_view,
    ...
    case STORAGE_LIST_MENU_FORMAT: {
        ...
        mui_msg_box_set_header(app->p_msg_box, "Formato");
        mui_msg_box_set_message(app->p_msg_box, "Se eliminarán todos los datos.\nConfirmar formato?");
        mui_msg_box_set_btn_text(app->p_msg_box, "Determinar", NULL, "Cancelación");
...
#define mount_str "===Montaje[LFS]==="
#else
#define mount_str "===Montaje[FFS]==="
#endif
        mui_list_view_add_item(app->p_list_view, 0xe1ca,
                               stat.avaliable ? mount_str : "=====Sin montar=====", (void *)-1);
        if (stat.avaliable) {
            snprintf(txt, sizeof(txt), "Espacio total: %d kB", stat.total_bytes / 1024);
            mui_list_view_add_item(app->p_list_view, 0xe1cb, txt, (void *)-1);
            snprintf(txt, sizeof(txt), "Espacio disponible: %d kB", stat.free_bytes / 1024);
            mui_list_view_add_item(app->p_list_view, 0xe1cc, txt, (void *)-1);
        }
    }

    mui_list_view_add_item(app->p_list_view, 0xe1cd, "Formato", (void *)STORAGE_LIST_MENU_FORMAT);

    mui_list_view_add_item(app->p_list_view, 0xe069, "Volver a la lista", (void *)STORAGE_LIST_MENU_BACK);
    mui_list_view_add_item(app->p_list_view, ICON_HOME, "Menú principal", (void *)STORAGE_LIST_MENU_BACK_MAIN_MENU);
