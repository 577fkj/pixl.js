...
static void amiibo_scene_storage_list_menu_on_selected(mui_list_view_event_t event, mui_list_view_t *p_list_view,
    ...
    case STORAGE_LIST_MENU_FORMAT: {
        ...
        mui_msg_box_set_message(app->p_msg_box, "Tutti i dati saranno cancellati.\nConferma la formattazione?");
        mui_msg_box_set_btn_text(app->p_msg_box, "conferma", NULL, "annulla");
...
#define mount_str "===Montato[LFS]==="
#else
#define mount_str "===Montato[FFS]==="
#endif
        mui_list_view_add_item(app->p_list_view, 0xe1ca,
                               stat.avaliable ? mount_str : "=====non montato=====", (void *)-1);
        if (stat.avaliable) {
            snprintf(txt, sizeof(txt), "Spazio totale: %d kB", stat.total_bytes / 1024);
            mui_list_view_add_item(app->p_list_view, 0xe1cb, txt, (void *)-1);
            snprintf(txt, sizeof(txt), "Spazio disponibile: %d kB", stat.free_bytes / 1024);
    ...
    mui_list_view_add_item(app->p_list_view, 0xe069, "Torna all'elenco", (void *)STORAGE_LIST_MENU_BACK);
    mui_list_view_add_item(app->p_list_view, ICON_HOME, "Menu principale", (void *)STORAGE_LIST_MENU_BACK_MAIN_MENU);
