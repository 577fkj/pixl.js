...
static void amiibo_scene_file_browser_menu_text_input_rename_folder_event_cb(mui_text_input_event_t event,
    ...
    if (event == MUI_TEXT_INPUT_EVENT_CONFIRMED) {
        ...
        if (renamed) {
            ...
            char msg[32];
            sprintf(msg, "Falló el cambio de nombre\nCódigo de error:%d", res);
...
static void amiibo_scene_file_browser_menu_on_selected(mui_list_view_event_t event, mui_list_view_t *p_list_view,
    ...
    case FILE_BROWSER_MENU_CREATE_FOLDER: {
        mui_text_input_set_header(app->p_text_input, "Nombre de la carpeta:");
    ...
    case FILE_BROWSER_MENU_CREATE_AMIIBO: {
        mui_text_input_set_header(app->p_text_input, "Nombre amibo:");
    ...
    case FILE_BROWSER_MENU_REMOVE_FOLDER: {
        char msg[64];
        snprintf(msg, sizeof(msg), "Eliminar %s ?", string_get_cstr(app->current_file));
        mui_msg_box_set_header(app->p_msg_box, "Eliminar");
        mui_msg_box_set_message(app->p_msg_box, msg);
        mui_msg_box_set_btn_text(app->p_msg_box, "Cancelación", NULL, "Eliminar");
    ...
    case FILE_BROWSER_MENU_RENAME_FOLDER: {
        mui_text_input_set_header(app->p_text_input, "Nuevo nombre:");
...
void amiibo_scene_file_browser_menu_on_enter(void *user_data) {
    ...
    mui_list_view_add_item(app->p_list_view, 0xe1c8, "Nueva carpeta..", (void *)FILE_BROWSER_MENU_CREATE_FOLDER);
    mui_list_view_add_item(app->p_list_view, 0xe1c8, "Nueva etiqueta..", (void *)FILE_BROWSER_MENU_CREATE_AMIIBO);
    if (string_cmp_str(app->current_file, "..") != 0) {
        mui_list_view_add_item(app->p_list_view, 0xe1c9, "Cambiar el nombre..",
                               (void *)FILE_BROWSER_MENU_RENAME_FOLDER);
        mui_list_view_add_item(app->p_list_view, 0xe1ca, "Eliminar..", (void *)FILE_BROWSER_MENU_REMOVE_FOLDER);
    }
    mui_list_view_add_item(app->p_list_view, 0xe069, "Lista de archivos", (void *)FILE_BROWSER_MENU_BACK_FILE_BROWSER);
    mui_list_view_add_item(app->p_list_view, 0xe069, "Menú principal", (void *)FILE_BROWSER_MENU_BACK_MAIN_MENU);
