...
static void amiibo_scene_file_browser_reload_folders(app_amiibo_t *app) {
    ...
    if (string_cmp_str(app->current_folder, "/") == 0) {
        ...
        mui_list_view_add_item(app->p_list_view, (one_driver && p_settings->skip_driver_select) ? ICON_HOME : ICON_BACK, (one_driver && p_settings->skip_driver_select) ? ">>Main menu<<" : "..", (void *)(one_driver && p_settings->skip_driver_select) ? -1 : FOLDER_LIST_PARENT);
    } else {
